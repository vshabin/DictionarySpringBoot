...
public class SessionRepository {
    ...
    public boolean exists(String refreshToken) {
        return dbServer.getDB().find(SessionEntity.class).where().eq(SessionEntity.REFRESH_TOKEN, refreshToken).exists();
    }

    public GeneralResultModel delete(String refreshToken) {
        try {
            dbServer.getDB().find(SessionEntity.class).where().eq(SessionEntity.REFRESH_TOKEN, refreshToken).delete();
        } catch (Exception e) {
    ...
    public GeneralResultModel update(UUID id, String refreshToken, Date refreshExpiresAt, String accessToken, Date accessTokenExpiresAt) {
        try {
            dbServer.getDB().update(SessionEntity.class).set(SessionEntity.REFRESH_TOKEN, refreshToken).set(SessionEntity.REFRESH_EXPIRES_AT, refreshExpiresAt).set(SessionEntity.ACCESS_TOKEN, accessToken).set(SessionEntity.ACCESS_EXPIRES_AT, accessTokenExpiresAt).where().eq(SessionEntity.ID, id).update();
        } catch (Exception e) {
            return new GeneralResultModel(DATABASE_TRANSACTION_ERROR_CODE, DATABASE_TRANSACTION_ERROR_MESSAGE + e.getMessage());
        }
        return null;
